...
5. `runtime/th105_transitions.jsonl` is the bounded staging corpus for
   ...
   New writers add an optional `learning_context` containing relative height,
   independent self wall/field position, learned enemy-action family, and exact
   offensive native action ID. It is attribution metadata, not a new action
   vocabulary, so existing action-schema-v4 rows stay compatible. Offline
   replay reconstructs the self-zone token from the already durable quantized
   P1 x coordinate for older rows.
